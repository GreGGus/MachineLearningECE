...
res_test=[];
	...
	waitbar(l/ 120,h,iteration);


	%% Base log -> Dichotomie

	%%  [TEA, TET, pmc]=apprend_pmc(vall_app,Ya,vall_test,Yt,0.01,{10,10});
	 	[TEA, TET, pmc]=apprend_pmc(vall_app,Ya,vall_test,Yt,0.01,{l});
	...
	RES = [RES;[l ,ErrorRateApp, ErrorRateTest]];

end


save dataSetTP3OneLayer.dat RES res_app res_test
